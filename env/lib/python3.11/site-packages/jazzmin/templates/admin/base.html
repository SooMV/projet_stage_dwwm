...
<div class="wrapper">
    ...
    {% if not is_popup %}
        ...
        {% if jazzmin_settings.show_sidebar %}
            ...
            <aside class="main-sidebar elevation-4  bg-black {{ jazzmin_ui.sidebar_classes }}" id="jazzy-sidebar">
                <a href="{% url 'admin:index' %}" class="brand-link bg-black-900 {{ jazzmin_ui.brand_classes }}" id="jazzy-logo">
                    ...
                    <span class="brand-text text-white font-light">{{ jazzmin_settings.site_brand }}</span>
                </a>

                <div class="sidebar bg-yellow-400">
                    <div class="user-panel mt-3 pb-3 mb-3 d-flex">
                        <div class="image">
                            {% if jazzmin_settings|has_jazzmin_setting:"user_avatar" %}
                                <img src="{% get_user_avatar request.user %}" width="160px" class="img-circle elevation-2 text-black" alt="User Image">
                            {% else %}
                                <i class="fas fa-inverse user-profile fa-user-circle text-black"></i>
                            {% endif %}
                        </div>
                        <div class="info text-black">
                            {% if perms|can_view_self %}
                                <a href="{% jazzy_admin_url request.user request.current_app|default:"admin" %}" class="d-block text-black">{{ request.user }}</a>
                            {% else %}
                                <span class="d-block text-black">{{ request.user }}</span>
                            {% endif %}
                        </div>
                    </div>
                        ...
                        <ul class="nav nav-pills nav-sidebar flex-column {{ jazzmin_ui.sidebar_list_classes }}" data-widget="treeview" role="menu" data-collapsible="false">
                            ...
                            <li class="nav-item">
                                <a href="{% url 'admin:index' %}" class="nav-link">
                                    <i class="nav-icon fas fa-th-large text-black"></i>
                                    <p class="text-black ">{% trans 'Dashboard' %}</p>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a href="{% url 'averageCart' %}" class="nav-link">
                                    
                                    <i class="nav-icon fas fa-chart-pie text-black"></i>
                                    <p class="text-black ">Analyse de données</p>
                                </a>
                            </li>

                            {% if jazzmin_settings.navigation_expanded %}
                                {% for app in side_menu_list %}
                                    <li class="nav-header text-black">{{ app.name }}</li>
                                    {% for model in app.models %}
                                        <li class="nav-item">
                                            {% if model.url %}
                                            <a href="{{ model.url }}" class="nav-link text-black">
                                                <i class="nav-icon {{ model.icon }} text-black"></i> 
                                                <p class="text-black">{{ model.name }}</p>
                                            </a>
                                        {% else %}
                                            <span class="nav-link disabled">
                                                <i class="nav-icon {{ model.icon }}"></i> <p class="text-black">{{ model.name }}</p>
                                            </span>
                                        {% endif %}
                                        </li>
                                    {% endfor %}
